...
# 事实提取清单（图片/视频/笔记共用）
FACT_CHECKLIST = """- 游戏名称、版本
- 活动/事件名称和类型（联动/更新/福利/赛事等）
- 活动时间、地点、参与方式、规则
- 涉及的英雄、皮肤、角色、道具、装备名称
- 奖励内容、获取条件、兑换方式
- 画面中的文字信息（公告、弹幕、评论区、UI数值）"""

IMAGE_EXTRACTION_PROMPT = f"""请仔细观察这张图片，按以下三个板块输出完整的图片内容分析。

## 第1板块：画面内容描述
用连贯的段落描述图片中的场景、人物、动作、氛围。
...
## 第3板块：细节与可讨论点
逐条列出画面中引人注目的细节，以及看到这张图后想评论的具体切入点：
- 人物表情、动作、反应
- 特写镜头中的道具或UI细节
- 弹幕/评论中有趣的内容
- "看到XX的时候..."
...
VIDEO_EXTRACTION_PROMPT = f"""以下是视频的语音转录文本：
{{transcript}}

以下是视频的关键画面帧。请结合语音和画面，按以下三个板块输出完整的视频内容分析。

## 第1板块：视频内容叙事
像一个现场观众，用连贯的段落像讲故事一样描述视频从头到尾的完整内容。包含开场氛围、核心事件展开、关键转折/高潮、主播情绪变化、结尾收束。
...
## 第3板块：细节与可讨论点
逐条列出画面中引人注目的视觉细节，以及看完后想评论的具体切入点：
- 人物的表情变化和反应
- 画面中的特写镜头（道具、UI、操作细节）
- 弹幕中的刷屏梗、有趣评论、玩家共识
- 意外或搞笑瞬间
...
"""
            ...
            prompt = f"""请阅读以下抖音图文笔记，提取其中与游戏相关的信息。{focus_instruction}
【笔记原文】
{text[:5000]}
...
请用中文逐条列出。"""
    ...
    def _summarize_transcript(self, transcript: str) -> str:
        ...
        prompt = f"""以下是视频的语音转录文本，提取其中与游戏相关的关键信息。{self._focus_instruction()}
